...
export async function addSource(page, input) {
        ...
        if (expectedUuid) {
            ...
            if (currentUuid && currentUuid !== expectedUuid) {
                ...
                return {
                    ...
                    message: `NotebookLM redirected to a different notebook (${currentUuid}) instead of ` +
 ...
 */
async function openAddSourceOverlay(page) {
...
async function pickSourceType(page, type) {
    const candidates = type === "url" ? Selectors.sources.sourceTypeUrl : Selectors.sources.sourceTypeText;
...
async function fillSourceContent(page, input) {
    const overlay = page.locator(Selectors.sources.overlayPane).first();
    ...
    if (!target) {
        throw new Error("Could not find an input field inside the Add-source overlay. " +
...
async function confirmInsert(page) {
    const overlay = page.locator(Selectors.sources.overlayPane).first();
 ...
 */
async function waitForOverlayToClose(page, timeoutMs = 30_000) {
    await page
        .locator(Selectors.sources.overlayPane)
        .first()
...
async function waitForSourceCountIncrease(page, before, timeoutMs = 90_000) {
 ...
 */
async function readDialogError(page) {
    ...
    for (const sel of errorSelectors) {
        try {
            ...
            if (!txt || txt.length > 240)
                continue;
            if (ICON_LEAKS.some((leak) => txt.includes(leak)))
                continue;
            return txt;
        }
        catch {

...
export async function extractCitations(page, answerText, format = "none") {
    if (format === "none") {
...
async function readCitationStubs(page) {
    try {
...
async function extractExcerpt(page, number) {
    try {
...
function formatAnswer(answer, citations, format) {
    if (format === "none" || citations.length === 0)
        return answer;
    switch (format) {
        case "json": {
        ...
        default: {
            const footnotes = citations
                .map((c) => `${c.marker} ${c.sourceName}${c.sourceText && c.sourceText !== c.sourceName ? ` — "${truncate(c.sourceText, 240)}"` : ""}`)
                .join("\n");
            return `${answer}\n\nSources:\n${footnotes}`;
        }
    }

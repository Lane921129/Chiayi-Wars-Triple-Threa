...
export async function generateAudioOverview(page, options = {}) {
    ...
    try {
        ...
        if (await audioGenerationInProgress(page)) {
            ...
            return {
                status: "in_progress",
                message: "Audio Overview generation is already running. Poll `get_audio_status` " +
        ...
        if (!waitForCompletion) {
            return {
                status: "started",
                message: "Audio Overview generation started. It typically takes 2–10 minutes. " +
...
async function waitForAudioReady(page, timeoutMs) {
 ...
 */
export async function getAudioStatusOnPage(page) {
    try {
        if (await audioIsReady(page)) {
            return { status: "ready" };
        }
...
async function audioIsReady(page) {
    return page
...
async function audioGenerationInProgress(page) {
    try {
 ...
 */
async function ensureStudioPanelExpanded(page) {
...
async function openAudioCustomiseDialog(page) {
 ...
 */
export async function downloadAudioOverview(page, destinationDir, preferredFileName = "notebooklm-audio.wav") {
    try {
        if (!(await audioIsReady(page))) {
            return {
                success: false,
                message: "No completed Audio Overview found. Trigger `generate_audio` first " +
...
async function clickFirstVisible(page, selectors, label) {
    ...
    throw new Error(`Could not find ${label} — selectors: ${selectors.join(" | ")}. ` +

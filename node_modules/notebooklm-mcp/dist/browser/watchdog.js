 ...
 */
export async function safeSleep(page, ms) {
 ...
 * pages — never throws.
 */
export async function pageIsAlive(page, budgetMs = 1_500) {
    if (page.isClosed())
        return false;
    try {
        await Promise.race([
            page.evaluate(() => true),
            new Promise((_, reject) => setTimeout(() => reject(new Error("health check timed out")), budgetMs)),
        ]);
        return true;
    }
    catch {

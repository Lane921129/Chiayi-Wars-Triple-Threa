...
import { getPreferredChannel, isChannelFailure, withChannel, } from "../browser/chromium-fallback.js";
import fs from "fs";
...
export class SharedContextManager {
    ...
    async recreateContext(overrideHeadless) {
        ...
        const tryLaunch = async (userDataDir) => {
            ...
            try {
                return await chromium.launchPersistentContext(userDataDir, withChannel(baseLaunchOptions, preferred));
            }
            catch (err) {
                if (preferred === "chrome" && isChannelFailure(err)) {
                    log.warning("⚠️  System Chrome failed to launch — falling back to bundled Chromium (set BROWSER_CHANNEL=chromium to skip this retry).");
                    return await chromium.launchPersistentContext(userDataDir, withChannel(baseLaunchOptions, "chromium"));
                }
                throw err;
            }
        };
    ...
    getContextId() {
        ...
        const guid = this.globalContext._guid;

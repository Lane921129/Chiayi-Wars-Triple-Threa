...
export class AuthManager {
    ...
    async performSetup(sendProgress, overrideHeadless) {
        ...
        try {
            ...
            let context;
            try {
                context = await chromium.launchPersistentContext(CONFIG.chromeProfileDir, withChannel(baseLaunchOptions, preferred));
            }
            catch (err) {
                if (preferred === "chrome" && isChannelFailure(err)) {
                    log.warning("⚠️  System Chrome failed to launch — falling back to bundled Chromium.");
                    context = await chromium.launchPersistentContext(CONFIG.chromeProfileDir, withChannel(baseLaunchOptions, "chromium"));
                }
                else {
                    throw err;
                }

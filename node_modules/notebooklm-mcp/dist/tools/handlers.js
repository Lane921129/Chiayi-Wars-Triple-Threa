...
function followUpReminderEnabled() {
...
export class ToolHandlers {
    ...
    async handleAskQuestion(args, sendProgress) {
        const { question, session_id, notebook_id, notebook_url, show_browser, browser_options, source_format = "none", } = args;
     ...
     */
    async resolveNotebookUrl(notebookId, notebookUrl) {
        if (notebookUrl)
            return notebookUrl;
        if (notebookId) {
            const nb = this.library.getNotebook(notebookId);
            if (!nb)
                throw new Error(`Notebook not found in library: ${notebookId}`);
            return nb.url;
        }
        const active = this.library.getActiveNotebook();
        return active?.url;

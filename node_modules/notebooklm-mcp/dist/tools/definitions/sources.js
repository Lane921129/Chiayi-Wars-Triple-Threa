...
const sharedNotebookTargeting = {
    session_id: {
        type: "string",
        description: "Reuse an existing browser session by id. Recommended when you have " +
    ...
    notebook_id: {
        type: "string",
        description: "Library notebook id (from `list_notebooks` / `search_notebooks`). " +
            "Defaults to the active notebook (see `select_notebook`) when omitted.",
    },
    notebook_url: {
        type: "string",
        description: "Direct NotebookLM URL — overrides `notebook_id`. Use for ad-hoc " +
...
export const addSourceTool = {
    name: "add_source",
    description: "Ingest a source into a NotebookLM notebook. Supports two source types " +
        "in v2.0:\n" +
    ...
    inputSchema: {
        ...
        properties: {
            type: {
                type: "string",
                enum: ["url", "text"],
                description: "`url` crawls the supplied website; `text` ingests `content` " +
                    "verbatim as a copied document.",
            },
            content: {
                type: "string",
                description: "When `type=url`: a fully-qualified URL (https://…). " +
                    "When `type=text`: the raw text body (any length up to NotebookLM's per-source word limit, ~500 k for free tier).",
            },
            title: {
                type: "string",
                description: "Display title shown in the source list. Optional — NotebookLM " +
...
export const generateAudioTool = {
    name: "generate_audio",
    description: "Trigger podcast-style Audio Overview generation for a notebook.\n\n" +
    ...
    inputSchema: {
        ...
        properties: {
            custom_prompt: {
                type: "string",
                description: "Optional focus prompt for the Audio Overview, e.g. \"Focus on the " +
                    "API authentication flow and skip pricing\". Passed into the " +
                    "NotebookLM \"Customize\" sub-dialog before generation starts.",
            },
            wait_for_completion: {
                type: "boolean",
                description: "If true, block until the audio tile is ready (up to `timeout_ms`). " +
                    "Default false — return immediately and let the caller poll " +
                    "`get_audio_status`.",
            },
            timeout_ms: {
                type: "number",
                description: "Only relevant when `wait_for_completion=true`. Maximum wait for " +
...
export const getAudioStatusTool = {
    ...
    description: "Non-blocking probe for the current Audio Overview state of a notebook.\n\n" +
...
export const downloadAudioTool = {
    name: "download_audio",
    description: "Save the completed Audio Overview to disk as a `.m4a` file. **Pre-" +
    ...
    inputSchema: {
        ...
        properties: {
            destination_dir: {
                type: "string",
                description: "Absolute directory path where the file is saved (created if " +
